...
/* 扩展变量 ------------------------------------------------------------------*/
/* 私有函数原形 --------------------------------------------------------------*/
static void DHT11_Mode_IPU(void);    // PA15输入
static void DHT11_Mode_Out_PP(void); // PA15输出
static uint8_t DHT11_ReadByte(void); // 字节读取PA15
 ...
 * 返 回 值: ERROR：  读取出错
 *           SUCCESS：读取成功
 * 说    明：8bit 湿度整数 + 8bit 湿度小数 + 8bit 温度整数 + 8bit 温度小数 +
 * 8bit 校验和
 */
uint8_t DHT11_Read_TempAndHumidity(DHT11_Data_TypeDef *DHT11_Data) {
  ...
  /*输出模式*/
  DHT11_Mode_Out_PP(); // PA5设置为输出模式，stm32发送信号到DHT11
  /*主机拉低*/
  DHT11_Dout_LOW(); // STM32向PA5引脚写低电平，表示主机的启动信号，持续18ms
  /*延时18ms*/
  Delay_ms(18);

  /*总线拉高 主机延时30us*/
  DHT11_Dout_HIGH(); // STM32向PA5引脚写高电平，持续30us
